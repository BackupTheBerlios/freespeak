    ...
    def build_language_table(self):
        ...
        for row in rows:
            try:
                ...
                self.language_table.append({"from":from_lang, "to":to_lang, "abb":abb})
            except:
                pass
    ...
    def translate(self, text):
        ...
        for lang in self.language_table:
            if lang["from"] == self.from_lang and lang["to"] == self.to_lang:
                abb=lang["abb"]
        text = text.replace(" ", "+")
        url = 'http://babelfish.altavista.com/tr?doit=done&intl=1&tt=urltext&trtext=%s&lp=%s' % (text, abb)
        try: query = urllib.urlopen(url) 
        except:
            gtk.threads_enter()
        ...
        result = result[result.index("<form action=\"http://www.altavista.com/web/results\""):]
